...
my %dispatch_table_diff_fortran = (
    ...
    'OpenMP sentinels not in column one' =>
      \&UMDP3::openmp_sentinels_in_column_one,
    ...
    'Used Subversion keyword substitution which is prohibited' =>
      \&UMDP3::svn_keyword_subst,
    'Used !OMP instead of !$OMP' => \&UMDP3::omp_missing_dollar,
    'Used #ifdef or #ifndef rather than #if defined() or #if !defined()' =>
      \&UMDP3::cpp_ifdef,
    'Presence of fortran comment in CPP directive' => \&UMDP3::cpp_comment,
    'Used an archaic fortran intrinsic function' =>
      \&UMDP3::obsolescent_fortran_intrinsic,
    'EXIT statements should be labelled' => \&UMDP3::exit_stmt_label,
    'Intrinsic modules must be USEd with an INTRINSIC keyword specifier' =>
      \&UMDP3::intrinsic_modules,
    'READ statements should have an explicit UNIT= as their first argument' =>
      \&UMDP3::read_unit_args,
);
...
my %dispatch_table_diff_c = (
    'Line longer than 80 characters' => \&UMDP3::line_over_80chars,
    'Line includes tab character'    => \&UMDP3::tab_detection,
'Fixed-width Integer format specifiers must have a space between themselves and the string delimiter (the " character)'
      => \&UMDP3::c_integral_format_specifiers,
);
...
'Used an _OPENMP && SHUM_USE_C_OPENMP_VIA_THREAD_UTILS if-def test in a logical combination with a third macro'
      => \&UMDP3::c_openmp_define_no_combine,
    'Used !defined(_OPENMP) rather than defined(_OPENMP) with #else branch' =>
      \&UMDP3::c_openmp_define_not,
'Used an omp #pragma (or #include <omp.h>) without protecting it with an _OPENMP if-def'
      => \&UMDP3::c_protect_omp_pragma,
    'Used the #ifdef style of if-def, rather than the #if defined() style' =>
      \&UMDP3::c_ifdef_defines,
    'C Unit does not end with a final newline character' =>
      \&UMDP3::c_final_newline,
);

my %dispatch_table_file_all =
  ( 'Line includes trailing whitespace character(s)' =>
      \&UMDP3::line_trail_whitespace, );
